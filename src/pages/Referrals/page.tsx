...
function ReferralNode({ node, isRoot = false }: { node: ReferralNode; isRoot?: boolean }) {
  return (
    <div className={`${isRoot ? '' : 'ml-8'} mb-4`}>
      <div className="flex items-center gap-3">
        <div className={`flex h-10 w-10 items-center justify-center rounded-full ${isRoot ? 'bg-green-500' : 'bg-blue-500'
          }`}>
...
export default function ReferralsPage() {
  return (
    <div className="rounded-xl bg-white/10 p-6 backdrop-blur-lg">
      <h1 className="mb-6 text-2xl font-bold text-white">Your Referral Network</h1>
      <div className="overflow-x-auto">
        <div className="min-w-[600px]">
          <ReferralNode node={referralData} isRoot={true} />
        </div>
      </div>
    </div>
  )
